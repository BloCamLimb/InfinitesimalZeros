	...
	public static void discoverAndLoadAnnotatedMaterialHandlers(ASMDataTable asmDataTable) {
        ...
        for(ASMData annotationData : annotations) {
            ...
            try {
                ...
                InfinitesimalZeros.logger.error("Failed to load annotated material handler class {}. FML annotation data is broken perhaps?", materialHandlerClassName);
                continue;
            }
            ...
            if(denyReason == null) {
                InfinitesimalZeros.logger.info("Registered material handler {}", materialHandler.getClass().getName());
                MaterialBasis.registerMaterialHandler(materialHandler);
                materialHandlersRegistered++;
            } else {
                InfinitesimalZeros.logger.error("Failed to load material handler class {} from {}: {}",
                    materialHandlerClassName, annotationData.getCandidate().getModContainer().getName(), denyReason, loadingError);
                InfinitesimalZeros.logger.error("Consult the mod author for fix on his side.");
            }
        }
        InfinitesimalZeros.logger.info("{} annotated material handlers registered", materialHandlersRegistered);
    }
}
