...
import infinitesimalzeros.api.interfaces.IItemModelIndexProvider;
...
public abstract class MetaItem<T extends MetaItem<?>.MetaValueItem> extends Item {
	...
	protected int getColorForItemStack(ItemStack stack, int tintIndex) {
		...
		IFluidHandlerItem fluidContainerItem = ItemHandlerHelper.copyStackWithSize(stack, 1).getCapability(CapabilityFluidHandler.FLUID_HANDLER_ITEM_CAPABILITY, null);
	...
	protected int getModelIndex(ItemStack itemStack) {
		...
		/*
		 * IElectricItem electricItem = itemStack.getCapability(GregtechCapabilities.CAPABILITY_ELECTRIC_ITEM, null); if(electricItem != null) { return (int)
		 * Math.min(((electricItem.getCharge() / (electricItem.getMaxCharge() * 1.0)) * 7), 7); }
		 */
		return 0;
	}
